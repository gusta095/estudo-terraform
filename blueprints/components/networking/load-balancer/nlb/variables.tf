...
variable "nlb_name" {
  ...
  validation {
    condition     = length(var.nlb_name) <= 32 && length(var.nlb_name) > 0 && can(regex("^[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]$", var.nlb_name))
...
variable "load_balancer_type" {
  ...
  validation {
    ...
    error_message = "O tipo do Load Balancer deve ser network"
  }
}
...
variable "subnets" {
  description = "Lista de IDs de subnets a serem associadas ao Load Balancer. Para Load Balancers do tipo `network`, as subnets não podem ser atualizadas. Alterar este valor em Load Balancers `network` resultará na recriação do recurso."
  type        = list(string)
  default     = null
}
